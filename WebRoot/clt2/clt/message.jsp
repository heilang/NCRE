    ...
    <!--导航栏-->
     ...
     <div class="box"> 
      <div class="main">
        ...
        <div id="message-list" class="content-list content-area">
          <p>资讯公告</p>
           <ul>
         ...
         //   System.out.print(cur_page);
            ...
            if(cur_page==null)
            {
            session.setAttribute("message_page","1");
            }
            else
            {
            session.setAttribute("message_page",cur_page);
            }
    
    //System.out.print(session.getAttribute("message_page"));
    
           ResultSet rs=connect.paging_message((String)session.getAttribute("message_page"),"messages");
            while(rs.next())
            {
           ...
           <div class="pages" id="pages">
            <ul>
            <%int page_cur=Integer.parseInt((String)session.getAttribute("message_page"));

            //System.out.println(page_cur); 
            %>
            
              <li id="previous"><span><a href='message.jsp?cur_page=<%if(page_cur>1) --page_cur;out.print(page_cur); %>'>Pre</a></span></li>
        
              <%
                      ResultSet rs_row=connect.messages("messages");
                      rs_row.last();
          ...
          if(page_yu!=0)
            ...
            all_page+=1;
              ...
              {
              out.print("<li><a href='message.jsp?cur_page="+(i+1)+"'>"+(i+1)+"</a></li>");
              }
              
               %>
           
             <%int page_curs=Integer.parseInt((String)session.getAttribute("message_page"));

            //System.out.println(page_cur); 
            %>
            
              <li id="next"><span><a href='message.jsp?cur_page=<% if(page_curs<all_page) page_curs++;out.print(page_curs);%>'>Next</a></span></li>
            </ul>
           </div>
        </div>
